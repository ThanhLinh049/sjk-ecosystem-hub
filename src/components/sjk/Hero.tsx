  ...
  return (
    <section className="relative min-h-[90vh] w-full overflow-hidden bg-[oklch(0.12_0.01_60)] text-white">
      ...
      <header className="relative z-10 mx-auto flex max-w-[1400px] items-center justify-between px-8 py-7">
        ...
        <a
          href="#contact"
          className="hidden rounded-full border border-white/20 bg-white/5 px-5 py-2.5 text-[12px] font-medium uppercase tracking-[0.18em] text-white backdrop-blur transition hover:border-[var(--brand-red)] hover:bg-[var(--brand-red)] md:inline-block"
        >
          Liên hệ hợp tác
        </a>
      </header>
      ...
      <div className="relative z-10 border-t border-white/10 bg-black/40 backdrop-blur-sm">
        <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-x-8 gap-y-2 px-8 py-4">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-[12px] text-white/70">
            {MEMBERS.map((m, i) => (
              <span key={m.id} className="flex items-center gap-6">
                <span className="flex items-center gap-2">
                  <span className="text-[10px] text-white/40">{m.index}</span>
                  {m.name}
                </span>
